...
export class UserCommand extends Command {
  public override async chatInputRun(interaction: CommandInteraction): Promise<void> {
    ...
    async function createCategory(name: string, description: string, emojiString: string) {
      ...
      await guildData.ticketCategories.push({ name, description, emoji: emojiString });
...
**Emoji:** ${emojiString}`),
    ...
    async function editCategory(oldName: string, newName: string, newDescription: string, newEmoji: string) {
      ...
      const Description = newDescription || current.ticketCategories.find((c) => c.name === oldName).description;
      const Emoji = newEmoji || current.ticketCategories.find((c) => c.name === oldName).emoji;
...
**Emoji:** ${Emoji}`),
    ...
    switch (interaction.options.getSubcommand(true)) {
      case 'add': {
        ...
        createCategory(name, description, emojiString);
        break;
      }
      case 'edit': {
        ...
        editCategory(oldName, newName, newDescription, newEmoji);
        break;
      }
      case 'delete': {
        const name = interaction.options.get('name')?.value as string;
