...
**Emoji:** ${emojiString}`),
    ...
    async function editCategory(
      ...
      const TicketText = newTicketText
        || current?.ticketCategories?.find((c) => c.name === oldName)?.ticketText
        || 'N/A' as string;
      const Description = newDescription
        || current?.ticketCategories?.find((c) => c.name === oldName)?.description as string;
      const EmbedDesc = newEmbedDesc
        || current?.ticketCategories?.find((c) => c.name === oldName)?.embedDesc as string;
      const Emoji = newEmoji
        || current?.ticketCategories?.find((c) => c.name === oldName)?.emoji as string;
